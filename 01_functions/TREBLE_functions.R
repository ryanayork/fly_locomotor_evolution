...
#Function for plotting morphospace filling in 2d
plot_morpho_filling_2d = function(pc1, 
                                  pc2, 
                                  dates, 
                                  contour = FALSE,
                                  plot = TRUE,
                                  normalize_pcs = FALSE,
                                  col = c(colorRampPalette(c('black', 'midnightblue'))(50), pals::parula(25)),
                                  cex = 1.5){
  
  pcs = data.frame(pc1 = pc1, 
                   pc2 = pc2, 
                   date = dates)
  pcs$id = apply(pcs, 1, function(x) paste(x[1:2], collapse = '_'))
  
  pcs = pcs[match(unique(pcs$id), pcs$id),]
  ...
  if(plot == FALSE){}else{
    ...
    cols = cols[match(pcs$date, names(cols))]
    
    if(contour == TRUE){
      dens =  kde2d(pcs[,1],
                    pcs[,2], n = 400,
                    h = 1, lims = c(c(min(pcs$pc1),
                                      max(pcs$pc1)),
                                    c(min(pcs$pc2),
                                      max(pcs$pc2))))
      ...
      levels = quantile(dens$z, prob = c(0.5))
      contour(dens, levels=levels, add=T, col = 'gray50', drawlabels = FALSE)
